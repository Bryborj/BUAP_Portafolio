import matplotlib.pyplot as plt
import numpy as np


# Tile de madera
def woodTile():
    # 1. Definir la paleta de colores de la madera (de más claro a más oscuro)
    PALETTE = {
        0: [0xD4, 0xA3, 0x73],  # Brillo / Highlight (Bordes iluminados)
        1: [0xBC, 0x8A, 0x5F],  # Color Base / Madera normal
        2: [0x8B, 0x5E, 0x34],  # Sombra Media / Vetras de la madera
        3: [0x60, 0x38, 0x13],  # Sombra Oscura / Oclusión de esquinas y bordes inferiores
    }

    # 2. Diseñar el mapa de píxeles (Tile 8x8)
    # Se aplica luz direccional (arriba-izquierda claro, abajo-derecha oscuro)
    tile_data = np.array([
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
    ])
    return tile_data, PALETTE

# Tile de piedra
def stoneTile():
    # 1. Definir la paleta de colores de la piedra (de más claro a más oscuro)
    PALETTE = {
        0: [0xD4, 0xA3, 0x73],  # Brillo / Highlight (Bordes iluminados)
        1: [0xBC, 0x8A, 0x5F],  # Color Base / Madera normal
        2: [0x8B, 0x5E, 0x34],  # Sombra Media / Vetras de la madera
        3: [0x60, 0x38, 0x13],  # Sombra Oscura / Oclusión de esquinas y bordes inferiores
    }

    # 2. Diseñar el mapa de píxeles (Tile 8x8)
    # Se aplica luz direccional (arriba-izquierda claro, abajo-derecha oscuro)
    tile_data = np.array([
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
    ])
    return tile_data, PALETTE

# Tile de pasto
def grassTile():
    # 1. Definir la paleta de colores de la pasto (de más claro a más oscuro)
    PALETTE = {
        0: [0xD4, 0xA3, 0x73],  # Brillo / Highlight (Bordes iluminados)
        1: [0xBC, 0x8A, 0x5F],  # Color Base / Madera normal
        2: [0x8B, 0x5E, 0x34],  # Sombra Media / Vetras de la madera
        3: [0x1A, 0x4D, 0x0E],  # Sombra Oscura / Oclusión de esquinas y bordes inferiores
    }

    # 2. Diseñar el mapa de píxeles (Tile 8x8)
    # Se aplica luz direccional (arriba-izquierda claro, abajo-derecha oscuro)
    tile_data = np.array([
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
    ])
    return tile_data, PALETTE

# Tile de tierra
def dirtTile():
    # 1. Definir la paleta de colores de la tierra (de más claro a más oscuro)
    PALETTE = {
        0: [0xD4, 0xA3, 0x73],  # Brillo / Highlight (Bordes iluminados)
        1: [0xBC, 0x8A, 0x5F],  # Color Base / Madera normal
        2: [0x8B, 0x5E, 0x34],  # Sombra Media / Vetras de la madera
        3: [0x60, 0x38, 0x13],  # Sombra Oscura / Oclusión de esquinas y bordes inferiores
    }

    # 2. Diseñar el mapa de píxeles (Tile 8x8)
    # Se aplica luz direccional (arriba-izquierda claro, abajo-derecha oscuro)
    tile_data = np.array([
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
    ])
    return tile_data, PALETTE

# Tile de agua
def waterTile():
    # 1. Definir la paleta de colores del agua (de más claro a más oscuro)
    PALETTE = {
        0: [0xD4, 0xA3, 0x73],  # Brillo / Highlight (Bordes iluminados)
        1: [0xBC, 0x8A, 0x5F],  # Color Base / Madera normal
        2: [0x8B, 0x5E, 0x34],  # Sombra Media / Vetras de la madera
        3: [0x60, 0x38, 0x13],  # Sombra Oscura / Oclusión de esquinas y bordes inferiores
    }

    # 2. Diseñar el mapa de píxeles (Tile 8x8)
    # Se aplica luz direccional (arriba-izquierda claro, abajo-derecha oscuro)
    tile_data = np.array([
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
        [0,1,2,3,2,1,0,1],
    ])
    return tile_data, PALETTE

# Convierte el tile de números a colores
def converterRGB(data, palette: dict):
    # 3. Convertir el mapa de índices en una imagen RGB real
    rgb_image = np.zeros((8, 8, 3), dtype=np.uint8)
    for i in range(8):
        for j in range(8):
            rgb_image[i, j] = palette[data[i, j]]
    return rgb_image

# Retorna el tile deseado
def getTile(x: int):
    arrayTiles = [woodTile(), stoneTile(), grassTile(), dirtTile(), waterTile()]
    return arrayTiles[x]

def DEBUG():
    print(f"¿Qué tile quieres visualizar?\n1. Madera\n2. Piedra\n3. Pasto\n4. Tierra\n5. Agua")
    x = int(input("Introduce el número del tile: ")) - 1

    # 4. Configurar y mostrar el gráfico con Matplotlib
    fig, ax = plt.subplots(figsize=(5, 5))

    data, palette = getTile(x)
    ax.imshow(converterRGB(data, palette), interpolation='nearest')

    # Configuración estética para desarrollo de videojuegos (Ver la cuadrícula de píxeles)
    #ax.set_xticks(np.arange(-0.5, 8, 1), minor=True)
    #ax.set_yticks(np.arange(-0.5, 8, 1), minor=True)
    #ax.grid(which='minor', color='black', linestyle='-', linewidth=0.5)
    #ax.tick_params(which='both', bottom=False, left=False, labelbottom=False, labelleft=False)

    plt.show()

DEBUG()